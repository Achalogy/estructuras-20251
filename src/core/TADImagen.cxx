...
Imagen::Imagen(std::string r_path, int r_ancho, int r_alto,
               int r_max_intensidad,
               std::vector<std::vector<int>> r_contenido) {
  // Verificar valores
  if (r_ancho < 0 || r_alto < 0)
    throw std::out_of_range("Dimensiones invalidas");
  if (r_max_intensidad < 0 || r_max_intensidad > 255)
    throw std::out_of_range(
        "Intensidad maxima invalida, debe estar entre 0 y 255");
  ...
  for (std::vector<int> i : r_contenido) {
    if (matriz_invalida) break;

    if (i.size() != r_ancho) matriz_invalida = true;
  }

  if (matriz_invalida)
    throw std::invalid_argument(
        "La dimension de la matriz no concuerda con los datos de alto y ancho");

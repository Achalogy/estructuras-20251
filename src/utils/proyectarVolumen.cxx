...
std::vector<int> lanzarRayo(Volumen *vol, std::vector<int> mov, int x, int y,
                            int z) {
  std::vector<int> nums;
  std::vector<Imagen> imagenes = vol->getImagenes();

  while (true) {
    if (x < 0 || x >= vol->getAncho() || y < 0 || y >= vol->getAltura() ||
        z < 0 || z >= vol->getNumeroImagenes())
      break;

    nums.push_back(imagenes[z].getContenido()[y][x]);
...
Imagen *proyectarVolumen(Volumen *vol, char direccion, std::string criterio) {
  ...
  for (int i = 0; i < H; i++) {
    for (int j = 0; j < W; j++) {
      ...
      if (direccion == 'z') rayo = lanzarRayo(vol, mov, j, i, 0);

      proyeccion[i][j] = calcularPixel(rayo, criterio);

      if (proyeccion[i][j] > M) M = proyeccion[i][j];
    }
  }
